...
final class Html {
    ...
    private static String css() {
        return "<style>"
                ...
                + ".graph-shell{background:var(--surface);border:1px solid var(--line);border-radius:10px;"
                + "margin:.55rem 0;box-shadow:var(--shadow);overflow:hidden;}"
                + ".graph-toolbar{display:flex;gap:.35rem;align-items:center;padding:.45rem;border-bottom:1px solid var(--line);"
                + "background:var(--raised);}"
                + ".graph-toolbar button{min-width:2.2rem;padding:.35rem .6rem;border:1px solid var(--line);border-radius:7px;"
                + "background:var(--surface);color:var(--fg);cursor:pointer;font-weight:600;font-size:.85rem;}"
                + ".graph-toolbar button:hover{border-color:var(--accent);color:var(--accent);}"
                + ".graph-stage{height:min(68vh,46rem);min-height:24rem;overflow:hidden;touch-action:none;cursor:grab;"
                + "background:#fff;}"
                + ".graph-stage:active{cursor:grabbing;}"
                + ".graph-stage svg{width:100%;height:100%;max-width:none;display:block;}"
    ...
    private static String script(View view) {
        return "<script nonce=\"" + esc(view.nonce()) + "\">(function(){"
                ...
                + "});});"
                // Graph pages keep the exported SVG static, then move its viewBox in
                // the explorer for pan/zoom. Without JS the same SVG remains visible.
                + "document.querySelectorAll('[data-graph-viewer]').forEach(function(viewer){"
                + "var stage=viewer.querySelector('.graph-stage'),svg=viewer.querySelector('svg');"
                + "if(!stage||!svg){return;}"
                + "if(!svg.getAttribute('viewBox')){"
                + "var w=parseFloat(svg.getAttribute('width'))||stage.clientWidth||800;"
                + "var h=parseFloat(svg.getAttribute('height'))||stage.clientHeight||500;"
                + "svg.setAttribute('viewBox','0 0 '+w+' '+h);}"
                + "var home=readBox(),drag=null;"
                + "function readBox(){var p=(svg.getAttribute('viewBox')||'0 0 1 1').trim().split(/[ ,]+/).map(Number);"
                + "return {x:p[0]||0,y:p[1]||0,w:p[2]||1,h:p[3]||1};}"
                + "function writeBox(b){svg.setAttribute('viewBox',[b.x,b.y,b.w,b.h].join(' '));}"
                + "function point(e){var r=svg.getBoundingClientRect(),b=readBox();"
                + "return {x:b.x+((e.clientX-r.left)/Math.max(1,r.width))*b.w,"
                + "y:b.y+((e.clientY-r.top)/Math.max(1,r.height))*b.h};}"
                + "function zoom(f,c){var b=readBox(),p=c||{x:b.x+b.w/2,y:b.y+b.h/2},nw=b.w*f,nh=b.h*f;"
                + "writeBox({x:p.x-(p.x-b.x)*f,y:p.y-(p.y-b.y)*f,w:nw,h:nh});}"
                + "viewer.querySelectorAll('[data-graph-action]').forEach(function(btn){"
                + "btn.addEventListener('click',function(){var a=btn.getAttribute('data-graph-action');"
                + "if(a==='zoom-in'){zoom(.8);}else if(a==='zoom-out'){zoom(1.25);}else{writeBox(home);}});});"
                + "stage.addEventListener('wheel',function(e){e.preventDefault();zoom(e.deltaY<0 ? .85 : 1.18,point(e));},"
                + "{passive:false});"
                + "stage.addEventListener('pointerdown',function(e){stage.setPointerCapture(e.pointerId);"
                + "drag={x:e.clientX,y:e.clientY,b:readBox()};});"
                + "stage.addEventListener('pointermove',function(e){if(!drag){return;}"
                + "var r=svg.getBoundingClientRect(),dx=(e.clientX-drag.x)/Math.max(1,r.width)*drag.b.w,"
                + "dy=(e.clientY-drag.y)/Math.max(1,r.height)*drag.b.h;"
                + "writeBox({x:drag.b.x-dx,y:drag.b.y-dy,w:drag.b.w,h:drag.b.h});});"
                + "stage.addEventListener('pointerup',function(){drag=null;});"
                + "stage.addEventListener('pointercancel',function(){drag=null;});"
                + "stage.addEventListener('keydown',function(e){var b=readBox(),step=e.shiftKey ? .18 : .08;"
                + "if(e.key==='+'||e.key==='='){zoom(.85);e.preventDefault();}"
                + "else if(e.key==='-'||e.key==='_'){zoom(1.18);e.preventDefault();}"
                + "else if(e.key==='0'){writeBox(home);e.preventDefault();}"
                + "else if(e.key==='ArrowLeft'){b.x-=b.w*step;writeBox(b);e.preventDefault();}"
                + "else if(e.key==='ArrowRight'){b.x+=b.w*step;writeBox(b);e.preventDefault();}"
                + "else if(e.key==='ArrowUp'){b.y-=b.h*step;writeBox(b);e.preventDefault();}"
                + "else if(e.key==='ArrowDown'){b.y+=b.h*step;writeBox(b);e.preventDefault();}});"
                + "});"
                + "})();</script>";
    }
}
